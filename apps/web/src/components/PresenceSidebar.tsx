...
export function PresenceSidebar({ me, presence, files }: Props) {
  ...
  return (
    <section className="space-y-1">
      ...
      {me && (
        <div className="px-2 py-1 text-xs text-zinc-300 flex items-center gap-2">
          <span className="size-2 rounded-full bg-brand-400" />
          <span className="truncate">{me.name}</span>
          <span className="text-zinc-600 ml-auto">you</span>
        </div>
      )}
      {others.length === 0 && (
        <div className="px-2 py-1 text-xs text-zinc-500">No friends here yet.</div>
      )}
      {others.map((p) => (
        <div key={p.userId} className="px-2 py-1 text-xs text-zinc-300 flex items-center gap-2">
          <span
          ...
          {p.fileId && fileById.has(p.fileId) && (
            <span className="text-zinc-500 ml-auto truncate">{fileById.get(p.fileId)}</span>
          )}
        </div>
      ))}
    </section>
